...
2. Visual Studio コード ウィンドウの右側で、上部のリボンの **[表示]** > **[拡張機能]** を選択し、**Microsoft 356 Agents Toolkit** を検索し、**[更新]** > **[拡張機能の再起動]** を選択し、**[新しいエージェント/アプリの作成]** を選択し、ドロップダウンで **[カスタム エンジン エージェント]**  > **[Basic Custom Engine Agent]** > **[Azure OpenAI]** を選択します。 

**注** ツールキットを更新した後、VS コードを閉じてから再度開く必要がある場合があります。
3. 画面の上部にある空白のボックスに、最初に次のように入力します。
   ...
   c. **[Azure Open AI デプロイ名]** に **「gpt-4o」** と入力 > **Enter** > **[JavaScript]** を選択します。 
...
5. 上記を完了したら、**:** の後の削除したテキストを、前のタスクで保存した値に置き換えます (注: 値は引用符で囲む必要があります)。
   ...
   c. **azureOpenAIDeploymentName** に **"gpt-4o"** と入力します
...
12. おめでとうございます! これで、エージェントに質問してテストできるようになりました。
